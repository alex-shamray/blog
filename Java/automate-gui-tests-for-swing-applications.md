...
The source tree of this example, together with test code and utilities, is available for download from [Resources](#learn-more-about-this-topic). Running "ant" in the top directory will build the example and run tests on it. When you use the `TestUtils` utility class with your own code, just put it in your test directory and use it from your JUnit test code.
